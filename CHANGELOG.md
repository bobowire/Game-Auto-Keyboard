...
### 新增
- ✨ **语音拼音辅助匹配**（设置 → 语音 tab 开关，默认关闭）
  - 开启后字符匹配之外再做一轮拼音匹配（单字音节为单位、忽略声调、多音字展开全部读音）
  - 救回 ASR 同音误识别（如"加血"被听成"加雪"/"佳雪"）
  - 仲裁规则：拼音轮须严格高于字符轮得分且不低于 0.5；打平时中文优先；关闭时行为与原来完全一致
  - 数字自动对齐（"副本2" ↔ "副本二"）、英文大小写不敏感
  - 匹配日志输出两轮得分（voice_debug.log），便于排查与调参

### 修复
- 🐛 修正 `script_matching_with_prefix` 测试中与打分规则矛盾的错误断言（历史遗留，此前即失败）
